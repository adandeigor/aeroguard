    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
      ...
      <Card className="bg-sky-50/20 backdrop-blur-sm border border-sky-200/30 shadow-xl hover:shadow-2xl transition-all duration-300  group relative overflow-hidden">
        ...
        <CardHeader className="pb-2 relative z-10">
          ...
          <CardDescription className="text-sky-700">At {airData.location} (simulated geolocation)</CardDescription>
        </CardHeader>
        <CardContent className="pt-0 space-y-4 relative z-10">
          <div className="flex items-center justify-between">
            <Badge className={`${getAQIColor(airData.aqi)} text-white text-lg shadow-md`}>
              {airData.aqi}
            </Badge>
            {getAQIEmoji(airData.aqi)}
          </div>
          <p className="text-sm text-sky-600">PM2.5: {airData.pm25} µg/m³</p>
          <p className="text-xs text-sky-500">Updated: {airData.timestamp.toLocaleTimeString()}</p>
        </CardContent>
      </Card>
      ...
      <Card className="bg-sky-50/20 backdrop-blur-sm border border-sky-200/30 shadow-xl hover:shadow-2xl transition-all duration-300  group relative overflow-hidden">
        ...
        <CardHeader className="pb-2 relative z-10">
          ...
          <CardDescription className="text-sky-700">At {airData.location}</CardDescription>
        </CardHeader>
        <CardContent className="pt-0 space-y-4 relative z-10">
          <div className="flex items-center justify-between">
            <span className="text-3xl font-bold text-sky-950">{weatherData.temperature}°C</span>
      ...
      <Card className="bg-sky-50/20 backdrop-blur-sm border border-sky-200/30 shadow-xl hover:shadow-2xl transition-all duration-300  md:col-span-2 relative overflow-hidden">
        ...
        <CardHeader className="relative z-10">
          ...
          <CardDescription className="text-sky-700">Full breakdown for {airData.location}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 relative z-10">
          ...
          <div>
            <h3 className="text-sky-800 font-semibold mb-2">Health Recommendations</h3>
